...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗦𝞝𝗗𝙏𝙃𝙊𝙉 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/E9N99) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗦𝞝𝗗─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 1488114134 or user.id == 1488114134 or user.id == 1488114134:
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗦𝞝𝗗 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/E9N99) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 1488114134 or user.id == 1488114134 or user.id == 1488114134
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا بلال الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
